...
class TestClient(fl.client.NumPyClient):
    ...
    def plot_and_save_history2(self):
        ...
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.title(f"Training History Rounds{num}")
        plt.legend()
        # Save the plot
        plt.savefig(f"training_history_round{num}.png")
        plt.close()
...
def create_model(input_shape):
    ...
    model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001), 
                  loss=tf.keras.losses.BinaryFocalCrossentropy(alpha=0.1, gamma=2), 
...
dataset = pd.read_csv('4152.csv', index_col='ts', parse_dates=True)
